...
def experiment_epsilon_decay():
    ...
    """
    Q1, r1, _ = train(algorithm="qlearning", episodes=600,
                      epsilon_start=0.1, epsilon_end=0.1, epsilon_decay=1.0)
    Q2, r2, _ = train(algorithm="qlearning", episodes=600,
                      epsilon_start=0.5, epsilon_end=0.5, epsilon_decay=1.0)
    Q3, r3, e3 = train(algorithm="qlearning", episodes=600,
...
def experiment_algorithms():
    ...
    """
    Q_ql, r_ql, _ = train(algorithm="qlearning", episodes=600, alpha=0.1, gamma=0.9,
                           epsilon_start=1.0, epsilon_end=0.05, epsilon_decay=0.995)
    Q_sa, r_sa, _ = train(algorithm="sarsa",     episodes=600, alpha=0.1, gamma=0.9,
                           epsilon_start=1.0, epsilon_end=0.05, epsilon_decay=0.995)
    ...
    print("\nQ-values near the hole:")
    for s, loc in hole_neighbors:
        print(f"  State {s:2d} ({loc:>14s}) | Q-learning max: {np.max(Q_ql[s]):.3f} | SARSA max: {np.max(Q_sa[s]):.3f}")

    print(f"\nQ-learning | final 50-ep avg: {np.mean(r_ql[-50:]):.2f}")
    print(f"SARSA      | final 50-ep avg: {np.mean(r_sa[-50:]):.2f}")

    # SARSA has lower Q-values near the hole because it is on-policy: during
    # training it uses the same epsilon-greedy policy for both action selection
    # and the TD target. Since the exploring policy occasionally steps into the
    # hole, those dangerous transitions are backed up into adjacent states,
    # depressing their values. Q-learning is off-policy and bootstraps from
    # max Q (the greedy action), so it ignores the exploratory steps into the
    # hole and assigns higher values to hole-adjacent states.
...
def replay_episode(Q, epsilon=0.0, max_steps=50):
    ...
    while not done and step < max_steps:
        ...
        print(f"Step {step:2d} | State: {state:2d} (row {row}, col {col}) | Action: {action_name:<5s} | Reward: {reward:.1f}")
        total_reward += reward
        state = next_state
