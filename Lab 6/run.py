...
def experiment_epsilon_decay():
    ...
    Q1, rewards_log1, epsilon_log1 = train("qlearning", 600, 0.1, 0.9, 0.1, 0.1, 1.0)
    Q2, rewards_log2, epsilon_log2 = train("qlearning", 600, 0.1, 0.9, 0.5, 0.5, 1.0)
    Q3, rewards_log3, epsilon_log3 = train("qlearning", 600, 0.1, 0.9, 1.0, 0.01, 0.995)

    fig, ax = plt.subplots(1,2, figsize = (12,4))

    ax[0].plot(smooth(rewards_log1), label = 'Fixed ε = 0.1')
    ax[0].plot(smooth(rewards_log2), label = 'Fixed ε = 0.5')
    ax[0].plot(smooth(rewards_log3), label = 'Decaying ε')

    ax[0].axhline(y = 0, color = 'black', linestyle = '--')
    ax[0].set_title("Smoothed Reward Curves")
    ax[0].set_xlabel("Episode")
    ax[0].set_ylabel("Smoothed reward")
    ax[0].legend()

    ax[1].plot(epsilon_log3, color = 'blue')
    ...
    print(f"Fixed 0.1 | final 50-ep avg: {np.mean(rewards_log1[-50:]):.2f}")
    print(f"Fixed 0.5 | final 50-ep avg: {np.mean(rewards_log2[-50:]):.2f}")
    print(f"Decaying ε | final 50-ep avg: {np.mean(rewards_log3[-50:]):.2f}")
...
def experiment_algorithms():
    ...
    """
    Q1, rewards_log1, epsilon_log1 = train("qlearning", 600, 0.1, 0.9, 1.0, 0.05, 0.995)
    Q2, rewards_log2, epsilon_log2 = train("sarsa", 600, 0.1, 0.9, 1.0, 0.05, 0.995)

    plt.figure()
    plt.plot(smooth(rewards_log1), label = 'Q Learning')
    plt.plot(smooth(rewards_log2), label = 'SARSA')

    plt.legend()
    plt.savefig("algorithm_comparison.png")
    print_policy(Q1, label = "Q-Learning Policy")
    print_policy(Q2, label = "SARSA Policy")

    labels = {2: "Above", 5: "Left", 7: "Right", 10: "Below"}

    for s in [2, 5, 7, 10]:
        print(f"State {s} ({labels[s]})| QL: {np.max(Q1[s]):.2f} | SA: {np.max(Q2[s]):.2f}")

    print(f"QL Final Avg: {np.mean(rewards_log1[-50:]):.2f}")
    print(f"SA Final Avg: {np.mean(rewards_log2[-50:]):.2f}")

    # SARSA because it's "on policy" so it so it learns from the actions it takes, 
    # knowing how bad it is to fall into the hole during training. Therefore, its values are lower
    # to sway SARSA away from the hole. However, Q Learning thinks that since it always is 
    # trying to find the optimal way out, it will never get near the hole, leading its values
    # to be higher.
...
def replay_episode(Q, epsilon=0.0, max_steps=50):
    ...
    while not done and step < max_steps:
        ...
        print(f"Step {step:>2} | State: {state:>2} (row {row}, col {col}) | Action: {action_name:<5} | Reward: {reward:4.1f}")

        state = next_state
        total_reward += reward
    
    goal_reached = (state == GridWorld.GOAL)
    print(f"Episode finished in {step} steps. Total Reward: {total_reward:.2f}. Reached goal: {goal_reached}")
