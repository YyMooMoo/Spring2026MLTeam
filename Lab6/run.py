...
def experiment_epsilon_decay():
    ...
    Q1, r1, e1 = train(algorithm="qlearning", episodes=600,
                        epsilon_start=0.1, epsilon_end=0.1, epsilon_decay=1.0)
    Q2, r2, e2 = train(algorithm="qlearning", episodes=600,
...
def experiment_algorithms():
    ...
    """
    kwargs = dict(episodes=600, alpha=0.1, gamma=0.9,
    ...
    for s, loc in hole_neighbors.items():
        print(f"  State {s:2d} ({loc:15s}) | Q-learning max: {np.max(Q_ql[s]):.3f} | SARSA max: {np.max(Q_sa[s]):.3f}")

    print(f"\nQ-learning | final 50-ep avg: {np.mean(r_ql[-50:]):.2f}")
    print(f"SARSA      | final 50-ep avg: {np.mean(r_sa[-50:]):.2f}")

    # SARSA tends to have lower Q-values near the hole because it is on-policy:
    # it evaluates the policy it actually follows (including exploratory actions),
    # so risky states near the hole get penalized more. Q-learning is off-policy
    # and always bootstraps from the greedy max, which can be overoptimistic near
    # dangerous states.
...
def replay_episode(Q, epsilon=0.0, max_steps=50):
    ...
    while not done and step < max_steps:
        ...
        if done and next_state == GridWorld.GOAL:
            reached_goal = True
        state = next_state
